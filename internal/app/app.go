...
// StartConsumer 启动消费者循环(由 api 进程调用, 阻塞直到 ctx 取消)。
// worker 每次消费动态取最新 store/registry(Reinit 后生效)。

...
- 深层文档: internal/{config,db,models,queue,store,worker}/AGENTS.md 记录各自核心语义与踩坑(配置契约/方言分支/数据契约/租约/惰性 getter)。
...
| compat | 兼容层: Server酱3(sc 形态)/ Server酱·Turbo版(sctapi 形态) | 公开路由, 走兼容 key 鉴权(非死目录); 存储常量 serverchan_v1/v2 不变 |
| config | 配置加载与持久化 + 后台路径校验/随机生成 | MP_* 环境变量 > config.yaml > 默认值; Save 由安装程序调用; 见 config/AGENTS.md |
| db | GORM 初始化 + AutoMigrate | PostgreSQL / MySQL 5.7 / SQLite(配置切换); SQLite 单写连接; 旧库迁移见 db/AGENTS.md |

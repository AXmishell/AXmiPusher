...
- Breaker 双实现: 内存 CircuitBreaker(单机) + RedisCircuitBreaker(多实例)。状态 0 闭合 / 1 熔断 / 2 半开, 按 (租户, 渠道) 维度隔离。
- Redis 熔断: 状态存 breaker:{tenant}:{channel} HASH, 全部状态转移走 Lua 原子脚本, 保证多实例并发正确。
- 新增渠道 = 实现 Sender + NewXxxSender 构造 + 在组装处 Register。无插件机制。

## 配置优先级
- 租户自定义: models.Channel 表, 按 tenant_id + type + status=active 查, Config 字段 JSON 解析成各渠道 Config。
- email 特例: 租户配置缺失时降级读平台 Settings("smtp" key)。apns / fcm 只认租户配置。
- 结论: 租户 Channel(type 维度, 租户覆盖) > 平台 Settings(SMTP)。
...
- inapp: 写 InappMessage 表; Recipient 为邮箱或 "all"(全租户用户)。

...
## OVERVIEW
部署/安装/分发目录: 本地仅写代码 → git 推送 → 云端编译部署(cloud-build-deploy.sh); 另提供安装包打包(install.sh/pack-install.ps1, 分发用)。
...
| 打包安装包 | pack-install.ps1 | 输出 messagepusher-install-{ver}.tar.gz(需先 build-artifacts.ps1) |

...
## CONVENTIONS
- **云端编译工作流**(2026-08 起): 本地仅写代码, `git push cloud deploy` → `sudo bash /opt/axmipusher-src/deploy/cloud-build-deploy.sh`。产物不再本地构建。
...
- install.sh 的 binary 模式服务名 `axmipusher`(已避开与旧 systemd 部署的重名混淆)
